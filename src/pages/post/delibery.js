...
searchBtn.addEventListener("click", () => {
    ...
    if (keyword === "") {   // 검색어 입력 안 할 시 검색 불가
        ...
        const iframeSrc = `../board/list.html?category=delibery&subCategory=${subCategoryText}`;
        iframe.src = iframeSrc;
        setIframeAndSaveState(iframeSrc);  // 상태 저장 및 iframe 설정(뒤로 가기 용도)
        ...
        const iframeSrc = `../board/list.html?category=delibery&subCategory=${subCategoryText}&search=${encodeURIComponent(keyword)}`;
        iframe.src = iframeSrc;
        setIframeAndSaveState(iframeSrc);  // 상태 저장 및 iframe 설정(뒤로 가기 용도)
    }
});